...
namespace BAIO.Heroes.Base
{
    ...
    internal class EmberSpirit : BaseHero
    {
        ...
        {
            if (Game.IsPaused || !this.Owner.IsAlive || !this.Fist.CanBeCasted)
            {
            ...
            {
                var output = this.Context.Prediction.GetPrediction(FistPredictionInput(killstealTarget));
                if (this.Fist.UseAbility(output.CastPosition))
                {
