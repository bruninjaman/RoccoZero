...
using Divine.BeAware.Overlay.SpellModes;
using Divine.BeAware.Utils;
    ...
    internal abstract class Overlay
    {
        ...
        private void OnRendererManagerDraw()
        {
            try
            {
                ...
                foreach (var hero in Heroes)
                {
                    ...
                    if (topPanel != null)
                    {
                        if (TopPanelMenu.HealthManaBarItem && hero.IsAlive)
                        {
                            ...
                            RendererManager.DrawTexture("Divine.BeAware.Resources.Textures.topbar_health.png", new RectangleF(healthPosition.X, healthPosition.Y, topPanel.HealthSizeX(healthPerc), topPanel.HealthSizeY));

                            var manaPosition = topPanel.ManaPosition;
                            RendererManager.DrawTexture("Divine.BeAware.Resources.Textures.topbar_mana.png", new RectangleF(manaPosition.X, manaPosition.Y, topPanel.ManaSizeX(manaPerc), topPanel.ManaSizeY));
                        }
                        ...
                        if (TopBarUltimateOverlay)
                        {
                            ...
                            if (ultimate != null)
                            {
                                ...
                                if (cooldown > 0)
                                {
                                    ...
                                    RendererManager.DrawTexture("Divine.BeAware.Resources.Textures.topbar_ulti.png", new RectangleF(ultimateBackPosition.X, ultimateBackPosition.Y, ultimateBackSize.X, ultimateBackSize.Y));
                ...
                foreach (var hero in Heroes.ToArray())
                {
                    ...
                    RendererManager.DrawTexture($"Divine.BeAware.Resources.Textures.{topPanel.VisibleIconName}.png", topPanel.VisibleIconRectangle);
                }
            }
            catch (Exception e)
            {
        ...
        private void DrawTownPortalScroll(Hero hero, Vector2 hpBarPosition, float mana)
        {
            ...
            if (tp != null && tp.IsValid)
            {
                ...
                RendererManager.DrawTexture(@"items\item_tpscroll.png", new RectangleF(position.X, position.Y, size.X, size.Y));
