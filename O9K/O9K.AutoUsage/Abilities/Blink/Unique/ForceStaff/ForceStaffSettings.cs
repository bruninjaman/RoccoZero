...
internal class ForceStaffSettings
{
    private readonly MenuHeroToggler heroes;

    public ForceStaffSettings(Menu settings, IBlink ability)
    {
        var menu = settings.GetOrAdd(new Menu(ability.DisplayName, ability.Name).SetTexture(ability.Name));

        this.UseWhenMagicImmune = menu.GetOrAdd(new MenuSwitcher("Magic immune", false));
        this.UseWhenMagicImmune.SetTooltip("Use ability when hero is magic immune");
        ...
        this.UseOnEnemy = menu.GetOrAdd(new MenuSwitcher("Use on enemy", false));
        this.UseOnEnemy.SetTooltip("Use ability on enemy");
        this.UseOnEnemy.AddTranslation(Lang.Ru, "Использовать на врага");
        this.UseOnEnemy.AddTooltipTranslation(Lang.Ru, "Использовать способность на врага");
        this.UseOnEnemy.AddTranslation(Lang.Cn, "用于敌人");
        this.UseOnEnemy.AddTooltipTranslation(Lang.Cn, "对敌人使用能力");

        this.HpThreshold = menu.GetOrAdd(new MenuSlider("HP% threshold", 40, 1, 100));
        this.HpThreshold.SetTooltip("Use when hp% is lower");
        ...
        this.Distance = menu.GetOrAdd(new MenuSlider("Distance", 450, 0, 2000));
        ...
        this.EnemiesCount.SetTooltip("Use only when there are equals/more enemies near");
    ...
    public MenuSwitcher UseOnEnemy { get; }
