...
internal class Vambrace : IModule
{
    ...
    private Core.Entities.Abilities.NeutralItems.Vambrace vambrace;

    private bool subscribed;

    private bool switchingThreads;

    public Vambrace(IMainMenu mainMenu, IOrderSync orderSync)
    {
        this.orderSync = orderSync;
        this.eventManager = Context9.AssemblyEventManager;

        var menu = mainMenu.AutoActionsMenu.Add(new Menu(Divine.Helpers.LocalizationHelper.LocalizeName(AbilityId.item_vambrace), "Vambrace"));
    ...
    public void Dispose()
    {
        ...
        OrderManager.OrderAdding -= this.OnVambraceOrderAdding;
    ...
    private int GetVambraceSwitchCount(Ability9 ability, Vector3 position)
    {
        var currentAttribute = this.vambrace.ActiveAttribute;
    ...
    private void OnAbilityAdded(Ability9 ability)
    {
        try
        {
            ...
            if (ability.Id == AbilityId.item_vambrace)
            {
                if (this.subscribed)
                {
                    return;
                ...
                this.vambrace = (Core.Entities.Abilities.NeutralItems.Vambrace)ability;
                this.defaultAttribute = this.vambrace.ActiveAttribute;
    ...
    private void OnAbilityRemoved(Ability9 ability)
    {
        try
        {
            ...
            if (ability.Handle != this.vambrace?.Handle)
            {
                return;
            }
    ...
    private void OnOrderAdding(OrderAddingEventArgs e)
    {
        try
        {
            ...
            switch (order.Type)
            {
                case OrderType.Cast:
                {
                    if (order.Ability.Id == AbilityId.item_vambrace)
                    {
                        return;
                    }
                    ...
                    if (this.VambraceSwitched(ability, false, isPlayerInput))
                    {
                        e.Process = false;
                    }
                ...
                case OrderType.CastPosition:
                {
                    ...
                    if (this.VambraceSwitched(ability, order.Position, isPlayerInput))
                    {
                        e.Process = false;
                    }
                ...
                case OrderType.CastTarget:
                {
                    ...
                    if (this.VambraceSwitched(ability, (Unit)order.Target, isPlayerInput))
                    {
                        e.Process = false;
                    }
                ...
                case OrderType.CastToggle:
                {
                    ...
                    if (this.VambraceSwitched(ability, true, isPlayerInput))
                    {
                        e.Process = false;
                    }
                ...
                case OrderType.CastRune:
                {
                    ...
                    if (this.VambraceSwitched(ability, (Rune)order.Target, isPlayerInput))
                    {
                        e.Process = false;
                    }
    ...
    private void OnVambraceOrderAdding(OrderAddingEventArgs e)
    {
        try
        {
            ...
            var order = e.Order;
            if (order.Type != OrderType.Cast || order.Ability.Handle != this.vambrace.Handle)
            {
                return;
            }
            ...
            if (!this.vambrace.CanBeCasted())
            {
                return;
            }

            this.vambrace.UseAbility();
            this.defaultAttribute = this.vambrace.ActiveAttribute;
            e.Process = false;
        }
        catch (Exception ex)
    ...
    private void OnUpdate()
    {
        try
        {
            ...
            if (!this.vambrace.IsValid || this.vambrace.ActiveAttribute == this.defaultAttribute || !this.vambrace.CanBeCasted())
            {
                return;
            }

            if ((this.vambrace.Owner.IsCasting || this.vambrace.Owner.IsInvulnerable || this.vambrace.Owner.IsInvisible) && !this.forceChangeBackSleeper)
            {
                return;
            }
    ...
    private bool VambraceSwitched(Ability9 ability, Unit target, bool isPlayerInput)
    {
        if (!this.ShouldChangeVambrace(ability))
        {
            return false;
        }

        var switchCount = this.GetVambraceSwitchCount(ability, target.Position);
        if (switchCount == 0)
        {
            return false;
        }
            ...
            async () =>
                {
                    try
                    {
                        await this.SwitchVambrace(switchCount);
    ...
    private bool VambraceSwitched(Ability9 ability, Vector3 position, bool isPlayerInput)
    {
        if (!this.ShouldChangeVambrace(ability))
        {
            return false;
        ...
        var switchCount = this.GetVambraceSwitchCount(ability, position);
        if (switchCount == 0)
        {
            return false;
        }
    ...
    private bool VambraceSwitched(Ability9 ability, Rune target, bool isPlayerInput)
    {
        if (!this.ShouldChangeVambrace(ability))
        {
            return false;
    ...
    private bool VambraceSwitched(Ability9 ability, bool toggle, bool isPlayerInput)
    {
        if (!this.ShouldChangeVambrace(ability))
        {
            return false;
        ...
        var switchCount = this.GetVambraceSwitchCount(ability, Vector3.Zero);
        if (switchCount == 0)
        {
            return false;
        }
    ...
    private bool ShouldChangeVambrace(Ability ability)
    {
        if (!this.vambrace.IsValid)
        {
            return false;
        }
        ...
        if (!this.vambrace.CanBeCasted(false) || this.vambrace.Owner.IsInvulnerable || this.vambrace.Owner.IsInvisible)
        {
            return false;
        }
    ...
    private async Task SwitchVambrace(int count)
    {
        this.switchingThreads = true;

        this.vambrace.ChangeExpectedAttribute(count == 1);

        for (var i = 0; i < count; i++)
        {
            this.vambrace.UseAbilitySimple();
