        ...
        where T : ActiveAbility, IInvokableAbility
    {
        ...
        public bool IsInvoked
        {
            get
            {
                ...
                return (this.invokeTime + 0.5f) > GameManager.RawGameTime;
            }
        }
        ...
        public bool Invoke(List<AbilityId> currentOrbs, bool queue, bool bypass, bool invokeIfOnLastPosition)
        {
            ...
            if (invoked)
            {
                this.invokeTime = GameManager.RawGameTime;
            }
        ...
        public void SetOwner(Unit9 newOwner)
        {
            ...
            var wexAbility = newOwner.GetAbilityById(AbilityId.invoker_wex)
                             ?? EntityManager.GetEntities<Ability>().FirstOrDefault(x => x.Id == AbilityId.invoker_wex);
            ...
            var quasAbility = newOwner.GetAbilityById(AbilityId.invoker_quas)
                              ?? EntityManager.GetEntities<Ability>().FirstOrDefault(x => x.Id == AbilityId.invoker_quas);
            ...
            var exortAbility = newOwner.GetAbilityById(AbilityId.invoker_exort)
                               ?? EntityManager.GetEntities<Ability>().FirstOrDefault(x => x.Id == AbilityId.invoker_exort);
