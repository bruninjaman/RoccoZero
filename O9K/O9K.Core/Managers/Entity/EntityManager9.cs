namespace O9K.Core.Managers.Entity
{
    ...
    public static class EntityManager9
    {
        ...
        public static IEnumerable<Tree> Trees
        {
            get
            {
                return EntityManager.GetEntities<Tree>().Where(x => x.IsAlive).ToArray();
            }
        }
        ...
        internal static IEnumerable<Ability> BaseAbilities
        {
            get
            {
                return EntityManager.GetEntities<Ability>().Where(x => x.IsValid);
            }
        }
        ...
        {
            foreach (var entity in EntityManager.GetEntities<Ability>())
            {
                try
                {
                    if (!entity.IsValid)
        ...
        {
            AddUnit(EntityManager.LocalHero);

            foreach (var entity in EntityManager.GetEntities<Unit>())
            {
                try
                {
        ...
        private static readonly UpdateHandler delayedEntityHandler;
