...
using InvokerAnnihilation.Exceptions;
...
public class SpecialData
{
    ...
    public SpecialData(Entity talentOwner, AbilityId talentId)
    {
        try
        {
            this.talent = (Ability) EntityManager.Entities.FirstOrDefault(x => (x is Ability ability) && ability.Id == talentId && ability.Owner?.Handle == talentOwner.Handle)!;
        ...
        catch
        {
            ...
            var ex = new BrokenAbilityException($"Broken ability exception: {talentId.ToString()}");
