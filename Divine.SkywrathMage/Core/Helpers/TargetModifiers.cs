...
namespace Divine.Core.Helpers
{
    ...
    {
        public TargetModifiers(Unit target, params string[] modifiers)
        {
            foreach (var modifier in target.Modifiers)
            {
                ...
                {
                    IsDuelAghanimsScepter = EntityManager.GetEntities<Hero>().Any(x => x.IsVisible && x.IsAlive && x.HeroId == HeroId.npc_dota_hero_legion_commander && x.HasAghanimsScepter());
                }
